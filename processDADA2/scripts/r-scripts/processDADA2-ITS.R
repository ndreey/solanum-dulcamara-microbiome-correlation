...
dir.create("amplicon_rds/ITS/", showWarnings = FALSE, recursive = TRUE)

# Path to demultiplexed reads
path_raw <- "amplicon_data/ITS/raw"
path_filt <- "amplicon_data/ITS/filt"
...
# ============ FILTER AND TRIM ============
if (file.exists("amplicon_rds/ITS/out.rds")) {
...
if (file.exists("amplicon_rds/ITS/seqtab.rds")) {
  ...
  saveRDS(seqtab, "amplicon_rds/ITS/seqtab.rds")
...
# ============ REMOVE CHIMERAS ============
if (file.exists("amplicon_rds/ITS/seqtab.nochim.rds")) {
  ...
  saveRDS(seqtab.nochim, "amplicon_rds/ITS/seqtab.nochim.rds")
...
# ============ TRACK READS ============
if (file.exists("amplicon_rds/ITS/track.rds")) {
  message("Loading existing read tracking from RDS")
  track <- readRDS("amplicon_rds/ITS/track.rds")
} else {
  ...
  saveRDS(track, "amplicon_rds/ITS/track.rds")
  message("Read tracking saved")
}
print(head(track))

# ============ ASSIGN TAXONOMY ============
if (file.exists("amplicon_rds/ITS/taxa.rds")) {
  message("Loading existing taxonomy from RDS")
  taxa <- readRDS("amplicon_rds/ITS/taxa.rds")
} else {
  message("Assign taxonomy")
  taxa <- assignTaxonomy(seqtab.nochim, "tax/sh_general_release_dynamic_s_all_10.05.2021.fasta", 
    multithread=8, tryRC=TRUE)
  saveRDS(taxa, "amplicon_rds/ITS/taxa.rds")
...
message("DADA2 pipeline complete! All objects saved to amplicon_rds/ITS/")